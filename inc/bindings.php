...
function masthead_register_bindings(): void {
	...
	register_block_bindings_source(
		...
		array(
			'label'              => esc_html__( 'Footer credit', 'masthead' ),
...
function masthead_get_footer_credit_value(): string {
	...
	$text = sprintf(
		...
		esc_html__( 'Built with %s.', 'masthead' ),
		$credit
	);
	 ...
	 */
	$text = (string) apply_filters( MASTHEAD_SLUG . '/footer_credit_text', $text ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
